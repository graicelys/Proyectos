...
require_once($CFG->dirroot. '/local/laika/classes/form/exact_question_form.php');

// Establecer contexto y configuración de la página
$context = context_system::instance();
$PAGE->set_context($context);
$PAGE->set_url(new moodle_url('/local/laika/capability/exact_question.php'));
...
if ($action == 'del') {
    ...
    $DB->delete_records('laika_exact_questions', array('id' => $id));
}
 
// Esto crea una instancia del formulario.
$messageform = new \local_laika\form\exact_questions_form();
if ($data = $messageform->get_data()) {
    ...
    $contentq = required_param('contentq', PARAM_TEXT);
    $answer_id = required_param('answer_id', PARAM_INT);
 
    // Formulario laika_answers.
    if (!empty($contentq)) { //no tocar que si guarda.
        $record = new stdClass;
        $record->contentq = $contentq;
        $record->answer_id = $answer_id;
        $record->enabled = true;
    
        $DB->insert_record('laika_exact_questions', $record);
        redirect($PAGE->url);
    }
}
...
echo" <a class='btn btn-primary' href='answer.php' role='button'>Ir a escribir respuestas para el chat</a>";
echo"<p></p>";
...
echo '<p></p>';
        ...
        echo '<h5>Se recomienda que se escriban las preguntas con mayúscula inicial.</h5>';
        echo '</div>';
    echo'<p></p>';
if (has_capability('local/laika:viewmessages', $context)) {

    $questionfields = \core_user\fields::for_name()->with_identity($context);
    $questionfieldssql = $questionfields->get_sql('u');

    $sql = "SELECT q.id,q.contentq, q.enabled, u.id AS answer_id, u.content {$questionfieldssql->select}
        FROM {laika_exact_questions} q
        INNER JOIN {laika_answers} u ON u.id = q.answer_id";
    ...
    foreach ($messages as $m) {
        ...
        echo html_writer::tag('p', format_text($m->contentq,  FORMAT_PLAIN), array('class' => 'card-text'));
        ...
        if ($deleteanypost) {
            ...
            new moodle_url(
                '/local/laika/capability/exact_question.php', 
